...
- "off_topic": boolean — true when the post is NOT about the listed stock, most commonly a crypto token that shares the $SYMBOL. Set true if ANY crypto tell is present: "CA" or a contract address (Solana addresses are 32-44 base58 chars, often referenced by their prefix); PumpFun/Moby/Raydium/Bags/BagsApp/TaskKash or any token-launchpad/DEX platform (an @platform handle next to the $symbol is a strong tell); "CTO"/community takeover; airdrop, holders/holder count, on-chain lore; SocialFi/engagement-farming token promos ("claim 500 $X", "earn $X", task/points/tip platforms, "gm to everyone holding $X"); crypto KOL names or cashtags (e.g. Ansem/$ANSEM/@blknoiz06); "MC"/market-cap talk in millions when the ticker context shows a company worth billions (memecoins measure success in $1M-$100M MC — "millions MC", "million coded", "$100 million potential" for a multi-billion company means the coin, not the stock). Also true when the symbol is used as ordinary trading slang, not this company ("SL and TP" = stop loss/take profit, "high OI", "3 DTE"). Plain hype about the actual stock (shares, squeeze, options, earnings) is NOT off_topic.
